...
class SafetyLevel(Enum):
    ...
    def __lt__(self, other):
        ...
        order = {SafetyLevel.LOW: 0, SafetyLevel.MEDIUM: 1, 
                SafetyLevel.HIGH: 2, SafetyLevel.STRICT: 3}
...
class ModelCapabilities(BaseModel):
    ...
    task_expertise: Dict[TaskType, float] = Field(
        ...
        description="Task expertise scores (0.0 to 1.0)"
    )
    context_length_optimal: Optional[int] = Field(
        None,
        description="Optimal context length for this model"
    )
    ...
    safety_level: SafetyLevel = Field(
        default=SafetyLevel.MEDIUM,
        description="Model safety level"
    )
    ...
    def validate_task_expertise_scores(cls, v):
        ...
        for task_type, score in v.items():
            if not 0.0 <= score <= 1.0:
                raise ValueError(f"Task expertise score for {task_type} must be between 0.0 and 1.0")
        return v
    
    @model_validator(mode='after')
    def validate_context_lengths(self):
        """Validate that optimal context length doesn't exceed max."""
        if self.context_length_optimal is not None and self.context_length_optimal > self.context_length_max:
...
class CapabilityRequirement(BaseModel):
    ...
    min_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Minimum task expertise score")
    context_length_needed: Optional[int] = Field(None, gt=0, description="Required context length")
    required_safety_level: Optional[SafetyLevel] = Field(None, description="Required safety level")
    requires_streaming: bool = Field(default=False, description="Whether streaming is required")
    requires_function_calling: bool = Field(default=False, description="Whether function calling is required")
...
class CapabilityScore(BaseModel):
    ...
    overall_score: float = Field(ge=0.0, le=1.0, description="Overall capability match score")
    task_match_score: float = Field(ge=0.0, le=1.0, description="Task expertise match score")
    context_length_score: float = Field(ge=0.0, le=1.0, description="Context length compatibility score")
    safety_score: float = Field(ge=0.0, le=1.0, description="Safety level compatibility score")
    feature_score: float = Field(ge=0.0, le=1.0, description="Feature compatibility score")
    language_score: float = Field(ge=0.0, le=1.0, description="Language support score")
    penalties: float = Field(default=0.0, ge=0.0, description="Penalties applied to score")
...
class CapabilityMatcher:
    ...
    def score_task_match(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> float:
        ...
        if task_score < requirement.min_score:
            # Heavy penalty for not meeting minimum - return significantly reduced score
            return task_score * 0.1  # 90% penalty for not meeting minimum
        
        return task_score
    
    def score_context_length_match(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> float:
        ...
        if needed <= optimal:
            ...
            return 1.0 - ((needed - optimal) / (max_capacity - optimal)) * 0.5
    
    def score_safety_match(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> float:
        ...
        if model_safety >= required_safety:
            return 1.0
        else:
            return 0.0
    
    def score_feature_match(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> float:
        ...
        if requirement.requires_function_calling and not capabilities.supports_function_calling:
            score -= 0.5
        
        return max(0.0, score)
    
    def score_language_match(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> float:
        """Score programming language support."""
        if not requirement.preferred_languages:
            return 1.0
        
        supported_count = sum(
            1 for lang in requirement.preferred_languages
            if capabilities.supports_language(lang)
        )
        
        return supported_count / len(requirement.preferred_languages)
    
    def calculate_score(self, capabilities: ModelCapabilities, requirement: CapabilityRequirement) -> CapabilityScore:
